...
class PhongMaterial(LightedMaterial):
    ...
    def __init__(self,
        ...
        if rotation_axis is not None:  # Check if rotation_axis is provided
            self.add_uniform("bool", "rotationAxis", rotation_axis)  # Add rotation_axis uniform
            self._rotation_axis = rotation_axis  # Set rotation axis attribute
            self.add_uniform("float", "rotationSpeed", rotation_speed)  # Add rotation speed uniform
            self._rotation_speed = rotation_speed  # Set rotation speed attribute
    ...
    def vertex_shader_code(self):
        return """
            ...
            uniform bool rotationAxis;  // Add rotation axis uniform
            uniform float rotationSpeed;  // Add rotation speed uniform
            ...
            void main()
            {
                ...
                if (rotationAxis) {  // Horizontal rotation
                    UV = vertexUV + vec2(time * rotationSpeed, 0);
                } else {  // Vertical rotation
                    UV = vertexUV + vec2(0, time * rotationSpeed);
                }
